...
import { markChapterComplete, isChapterComplete, loseHeart, restoreHearts, getUserStats } from '@/data/index';
...
export default function QuizPage() {
  ...
  // }, [subject, topic, chapter]);
   ...
   // Initialize AI quiz 
  useEffect(() => {
    if (!subject || !topic || !chapter) return;

    console.log("📌 Route Params:", subject, topic, chapter);

    generateQuiz(subject, topic, chapter).then(data => {
      console.log("📝 AI Quiz Data:", data);

      if (data && data.questions) {
        setQuestions(data.questions);
        const timeLimit = data.questions.length * 5 * 60;
        setQuizTimeLimit(timeLimit);
        setTimeLeft(timeLimit);
      }
    });
  ...
  // if ai quiz 
   if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-900">
        <p className="text-gray-600 dark:text-gray-300">Generating AI quiz...</p>
      </div>
    );
  }
